...
func InitializeConfigStateMachine(ctx context.Context, kvConn store.IKVStore) State {
	...
	askSetupManagingRole := NewState(
		...
		askSetupManagingRoleFunction,
	)
	handleSetupManagingRole := NewState(
		"handle-setup-managing-role",
		handleSetupManagingRoleFunction,
	)

	askSetupManagingRoleFromRoot := NewState(
		"ask-setup-managing-role",
		askSetupManagingRoleFunction,
	)
	handleSetupManagingRoleFromRoot := NewState(
		"handle-setup-managing-role",
		handleSetupManagingRoleFunction,
	)
	...
	askSetupManagingRoleFromRoot.AddState(`>role (.+)`, handleSetupManagingRoleFromRoot)

	handleSetupManagingRoleFromRoot.AddState(`>yes`, saveConfig)
	handleSetupManagingRoleFromRoot.AddState(`>no`, askSetupManagingRoleFromRoot)
